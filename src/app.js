...
import { normalizeAssistantText, escapeHtml } from './utils/text.js';
import { speakText, togglePauseSpeech, getSpeechStatus } from './utils/speech.js';
import { renderExplanationResult } from './utils/resultRenderer.js';
...
export function createApp(root) {
  ...
  function openHistoryItem(id) {
    ...
    const rendered = renderExplanationResult({ taskText: item.task, explanationText: item.result });
    stateApi.setRenderedResult(rendered.html);
    stateApi.setCopyText(rendered.copyText);
    router.go('solve');
    stateApi.setRoute('solve');
    view();
    toast('Объяснение открыто');
  }
  ...
  function syncVoiceControls() {
    ...
    const pauseIcon = pauseBtn?.querySelector('.icon-playpause');
  ...
  async function handleSolve() {
    ...
    if (resultBox) resultBox.innerHTML = '<p class="result-line muted">🤖 Думаю...</p>'; 
    ...
    try {
      ...
      const rendered = renderExplanationResult({ taskText: text, explanationText: result });
      stateApi.setRenderedResult(rendered.html);
      stateApi.setCopyText(rendered.copyText);
      ...
      stateApi.setResult(errorText);
      stateApi.setRenderedResult(`<p class="result-line">${escapeHtml(errorText)}</p>`);
      stateApi.setCopyText(errorText);
      view();
    }
  }
  ...
  function bindEvents() {
    ...
    if (copyBtn) {
      copyBtn.addEventListener('click', async () => {
        const text = stateApi.get().currentCopyText || stateApi.get().currentResult || '';
