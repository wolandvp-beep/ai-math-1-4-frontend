...
export function createApp(root) {
  ...
  async function handleSolve() {
    ...
    if (resultBox) resultBox.textContent = '⚡ Готовлю понятное объяснение...';
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('[data-example-fill]').forEach(btn => {
      btn.addEventListener('click', () => {
        const input = document.getElementById('taskInput');
        if (!input) return;
        input.value = btn.dataset.exampleFill || '';
        input.classList.remove('example-text');
        input.focus();
        stateApi.setDraft(input.value);
      });
    });
    ...
    if (voiceBtn) {
      voiceBtn.addEventListener('click', () => {
        const result = speakText(stateApi.get().currentResult);
        if (!result) toast('Озвучка недоступна');
        else if (result === 'resumed') toast('Озвучивание продолжено');
      });
    }
