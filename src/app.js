...
import { renderMathResult, createPlainTextFromPayload } from './utils/mathRenderer.js';
...
export function createApp(root) {
  ...
  function paintResultBox() {
    const resultBox = document.getElementById('resultBox');
    if (!resultBox) return;
    const state = stateApi.get();
    const payload = state.currentResultPayload;
    const text = state.currentResult || '';
    if (!String(text).trim()) {
      resultBox.textContent = resultBox.dataset.placeholder || 'Здесь появится объяснение и ответ.';
      return;
    }
    resultBox.innerHTML = renderMathResult(payload, text);
  }
  ...
  function openHistoryItem(id) {
    ...
    stateApi.setResult(item.resultText || item.result || '', item.resultPayload || null);
    router.go('solve');
  ...
  async function handleSolve() {
    ...
    try {
      ...
      const resultPayload = typeof raw === 'string' ? null : raw;
      const result = normalizeAssistantText(createPlainTextFromPayload(resultPayload, typeof raw === 'string' ? raw : ''));
      stateApi.setResult(result, resultPayload);
      ...
      stateApi.setResult('Ошибка: ' + error.message, null);
      view();
    }
  }
