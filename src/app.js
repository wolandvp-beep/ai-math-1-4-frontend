...
export function createApp(root) {
  ...
  function bindEvents() {
    ...
    document.querySelectorAll('[data-example-fill]').forEach(btn => {
      btn.addEventListener('click', () => {
        const value = btn.dataset.exampleFill || '';
    ...
    document.querySelectorAll('[data-clear-task]').forEach(btn => {
      btn.addEventListener('click', () => {
        stateApi.setDraft('');
        stateApi.setResult('');
        view();
    ...
    if (favoriteBtn) {
      favoriteBtn.addEventListener('click', () => {
        const state = stateApi.get();
        const current = state.history.find(item => item.task === state.draft && item.result === state.currentResult) || state.history[0];
        if (!current) return toast('Пока нечего добавлять');
        stateApi.toggleFavorite(current.id);
        view();
