...
export function createApp(root) {
  ...
  function openHistoryItem(id) {
    ...
    stateApi.setActiveHistoryItemId(item.id);
    router.go('solve');
    stateApi.setRoute('solve');
    view();
    toast('Объяснение открыто из истории');
  }
  ...
  async function handleSolve() {
    ...
    stateApi.setSolving(true);
    stateApi.setActiveHistoryItemId(null);
    view();
    ...
    try {
      ...
      stateApi.setSolving(false);
      stateApi.setResult('Не удалось подготовить объяснение. Попробуйте ещё раз.\n\nТехническая деталь: ' + error.message);
      view();
    }
  }
  ...
  function bindEvents() {
    ...
    if (taskInput) {
      taskInput.addEventListener('input', (e) => {
        ...
        if (stateApi.get().activeHistoryItemId) stateApi.setActiveHistoryItemId(null);
      });
    }
    ...
    if (historySearch) {
      historySearch.addEventListener('input', e => {
        ...
        stateApi.setHistoryQuery(value);
        clearTimeout(root.__historySearchTimer);
        root.__historySearchTimer = setTimeout(() => view(), 120);
      });
    }
