...
function buildDirectOperationExplanation(operation, explanationText) {
  ...
  const bodyLines = (parsed.bodyLines && parsed.bodyLines.length)
    ? parsed.bodyLines
    : buildOperationLeadLines(operation);
  const metaLines = [];
...
function prepareExplanationStructure(explanationText, operations, taskText) {
  ...
  if (!metaLines.some(line => /^Ответ:/i.test(line))) {
    ...
    if (computed) {
      metaLines.push(`Ответ: ${ensureSentenceEnding(computed)}`);
    }
  }
...
function buildFlowItems(bodyLineItems, blocks, options = {}) {
  const { showSyntheticStepHeaders = false, orderGuideBlock = null, operations = [], appendUnusedBlocks = true } = options;
  const flowItems = [];
...
function buildPreparedExplanationData({ explanationText, taskText }) {
  ...
  const preferModelBodyForSingleOperation = Boolean(String(explanationText || '').trim())
    && kind === 'expression'
    && operations.length === 1
    && bodyLineItems.length > 0;

  const flowItems = buildFlowItems(bodyLineItems, blocks, {
    showSyntheticStepHeaders: kind === 'expression' && operations.length > 1,
    orderGuideBlock,
    operations,
    appendUnusedBlocks: !preferModelBodyForSingleOperation
  });
