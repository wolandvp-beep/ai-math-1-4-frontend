...
function buildDirectOperationExplanation(operation, explanationText, taskText = '') {
  ...
  const adviceText = extractSectionValue(parsed.adviceLine, /^Совет:\s*/i) || defaultAdviceForOperation(operation);

  let bodyLines = [];
  if (operation.operator === '÷' && shouldUseColumn(operation) && Number(operation.a) >= 1000) {
