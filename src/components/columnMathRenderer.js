...
function normalizeDivisionNotes(notes) {
  ...
  trimmed.forEach((note, index) => {
    ...
    normalized.push({
      ...note,
      text: index === 0
        ? `Сначала выделяем первое неполное делимое ${current}.`
        : `Теперь выделяем следующее неполное делимое ${current}.`
    });

    if (nextTry > current) {
      normalized.push({
        ...note,
        text: `Подбираем цифру частного. Берём ${qDigit}, потому что ${qDigit} × ${divisor} = ${product}, а ${qDigit + 1} × ${divisor} = ${nextTry}, это уже больше.`
      });
    } else {
      normalized.push({
        ...note,
        text: `Подбираем цифру частного. Берём ${qDigit}, потому что ${qDigit} × ${divisor} = ${product}.`
      });
    }

    normalized.push({
      ...note,
      text: `Пишем ${qDigit} в частном и вычитаем: ${current} - ${product} = ${remainder}.`
    });
...
function buildOperationLeadLines(operation) {
  ...
  if (operation.operator === '+') {
    if (!useColumn) {
      ...
      return ['Нужно найти сумму.', 'Объединяем оба количества и считаем по порядку.'];
    }
    return [
      'Нужно найти сумму.',
  ...
  if (operation.operator === '-') {
    ...
    return [
      'Нужно найти разность.',
      'Записываем числа столбиком. Если единиц не хватает, занимаем 1 у соседнего разряда.'
    ];
  }
  ...
  if (operation.operator === '×') {
    ...
    return [
      'Нужно найти произведение.',
      'Записываем множители столбиком и начинаем умножение с единиц нижнего числа.'
    ];
  }
  ...
  if (!useColumn) {
    ...
    return ['Нужно найти частное.', 'Деление проверяем таблицей умножения или подбором.'];
  }

  return [
    'Нужно найти частное.',
    'Сначала выделяем первое неполное делимое, потом по шагам делим, умножаем, вычитаем и сносим следующую цифру.'
  ];
}
...
function buildInlineOperationResultLine(operation) {
  if (!operation) return '';
  const answerValue = computeOperationAnswerValue(operation);
  if (!answerValue) return '';
  const operatorText = operation.operator === '÷' ? ':' : operation.operator;
  return `${operation.a} ${operatorText} ${operation.b} = ${answerValue}`;
}
...
function buildDirectOperationExplanation(operation, explanationText, taskText = '') {
  ...
  let bodyLines = cleanedBody.length ? cleanedBody.slice(0, 8) : buildOperationLeadLines(operation);
  if (!bodyLines.some(line => bodyLineLooksLikeImmediateResult(line, operation))) {
    const inlineResult = buildInlineOperationResultLine(operation);
    if (inlineResult) {
      bodyLines = [...bodyLines, ensureSentenceEnding(inlineResult)];
    }
  }
...
function renderExplanationLine(item) {
  ...
  if (/^Правило:/i.test(line)) {
    return `<p class="result-line result-line--rule"${attrs}>${safe}</p>`;
  }
  if (/^(Краткое условие|Решение):/i.test(line)) {
