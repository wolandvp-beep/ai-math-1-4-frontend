  ...
  trimmed.forEach((note, index) => {
    ...
    normalized.push({
      ...
      text: index === 0
        ...
        : `Теперь берём следующее неполное делимое ${current}.`
    });
    ...
    if (nextPayload) {
      normalized.push({
        ...note,
        text: `Остаток ${remainder} меньше ${divisor}. Сносим следующую цифру и получаем ${Number(nextPayload.current)}.`
      });
    } else if (remainder === 0) {
      normalized.push({ ...note, text: 'Деление закончено без остатка.' });
    } else {
      normalized.push({ ...note, text: `Остаток ${remainder} меньше ${divisor}, поэтому деление закончено.` });
    }
  });
...
function buildOperationLeadLines(operation) {
  if (operation.operator === '+') {
    return shouldUseColumn(operation)
      ? ['Ищем сумму.', 'Записываем числа столбиком: единицы под единицами, десятки под десятками.']
      : ['Ищем сумму.', `Прибавляем ${operation.b} к ${operation.a}.`];
  }
  if (operation.operator === '-') {
    return shouldUseColumn(operation)
      ? ['Ищем разность.', 'Записываем числа столбиком: единицы под единицами, десятки под десятками.']
      : ['Ищем разность.', `Вычитаем ${operation.b} из ${operation.a}.`];
  }
  if (operation.operator === '×') {
    return shouldUseColumn(operation)
      ? ['Ищем произведение.', 'Записываем множители столбиком и начинаем умножать с единиц нижнего числа.']
  ...
  return shouldUseColumn(operation)
    ? ['Ищем частное.', 'Записываем деление уголком и работаем по шагам.']
