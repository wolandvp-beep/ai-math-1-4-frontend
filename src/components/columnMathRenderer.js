...
function looksLikeSchoolFractionChainLine(line, kind) {
  ...
  if (countFractionTokens(source) < 2 || !/=/.test(source)) return false;
  return /^(?:Пример:|Ответ:|Записываем:|\d+\s*\/\s*\d+)/i.test(source);
}
...
function renderInlineSchoolFraction(numerator, denominator, options = {}) {
  ...
  return `
    <span class="${classes}" aria-label="${escapeHtml(`${numerator}/${denominator}`)}">
      ${mark ? `<span class="school-fraction__mark">${escapeHtml(String(mark))}</span>` : ''}
...
function renderSchoolFractionEquationLine(text, context = {}) {
  ...
  const canUseVisualChain = Boolean(context?.fractionVisual) && /^(?:Пример:|Ответ:|Записываем:)/i.test(prefix);
  const bodyHtml = canUseVisualChain
    ? buildFractionVisualTokens(context.fractionVisual).map(renderFractionVisualToken).join('')
    : renderFractionEquationBody(bodyText);
  return `
    <span class="school-fraction-expression${canUseVisualChain ? ' school-fraction-expression--visual' : ''}">
