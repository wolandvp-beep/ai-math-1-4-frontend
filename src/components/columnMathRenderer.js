...
function flattenSchoolChain(node, operatorsToFlatten, operands, operators) {
  if (node?.type === 'binary' && operatorsToFlatten.includes(node.operator)) {
    ...
    flattenSchoolChain(node.right, operatorsToFlatten, operands, operators);
    return;
  }
  operands.push(node);
}
...
function evaluateParsedExpressionAst(node, steps) {
  ...
  for (let index = 0; index < operators.length; index += 1) {
    ...
    pushEvaluatedStep(steps, current, operator, right, node.index ?? index);
    current = result;
  }
...
function buildFlowItems(bodyLineItems, blocks, options = {}) {
  const { showSyntheticStepHeaders = false } = options;
  const flowItems = [];
  ...
  blocks.forEach((block, index) => {
    ...
    if (showSyntheticStepHeaders) {
      flowItems.push({
        type: 'line',
        item: { text: formatSyntheticStepHeader(block.operation, index + 1) }
      });
    }
    flowItems.push({ type: 'block', block });
  });
