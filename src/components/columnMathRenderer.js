...
function buildOrderGuideBlock(taskText, operations, lineRefs = {}) {
  ...
  const segments = shouldWrap ? segmentOrderGuideTokens(tokens, maxColumnsPerRow) : [{ tokens, columns: totalColumns }];
...
function renderInlineSchoolFraction(numerator, denominator) {
  return `
    <span class="school-fraction" aria-label="${escapeHtml(`${numerator}/${denominator}`)}">
...
function renderExplanationLine(item, context = {}) {
  ...
  const rich = renderTextWithSchoolFractions(line, kind);
  const attrs = typeof item?.lineIndex === 'number' ? ` data-result-line-index="${item.lineIndex}"` : '';
  if (/^Ответ:/i.test(line)) {
  ...
  if (/^(\d+)\)/.test(line.trim())) {
    ...
    return `<p class="result-line result-line--step${shouldRenderFractionsAsSchoolStack(content, kind) ? ' result-line--fraction' : ''}"${attrs}><span class="result-line-step__index" style="color:${color};">${escapeHtml(prefix)}</span> <span class="result-line-step__text">${renderTextWithSchoolFractions(content, kind)}</span></p>`;
  }
  if (looksLikeOrderMarkersLine(line)) {
