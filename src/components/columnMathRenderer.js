...
function prepareExplanationText(explanationText, operations, taskText) {
  if (!operations.length) {
    return explanationText;
  }

  return compactDivisionExplanation(explanationText, operations[0], taskText);
}
...
export function getPreparedExplanationText({ explanationText, taskText }) {
  ...
  return prepareExplanationText(String(explanationText || ''), operations, taskText);
}

function renderExplanationLine(line) {
  const safe = escapeHtml(line || '');
  if (/^Ответ:/i.test(line)) {
    return `<div class="result-line result-line--answer">${safe}</div>`;
  }
  if (/^Совет:/i.test(line)) {
    return `<div class="result-line result-line--advice">${safe}</div>`;
  }
  if (/^Проверка:/i.test(line)) {
    return `<p class="result-line result-line--check">${safe}</p>`;
  }
  return `<p class="result-line">${safe}</p>`;
}
...
function renderExplanationText(text) {
  ...
  return prepared
    ...
    .map(line => line.trim())
    .filter(Boolean)
    .map(renderExplanationLine)
    .join('');
}

export function renderExplanationWithColumns({ explanationText, taskText }) {
  const operations = collectOperations(taskText, explanationText);
  const preparedExplanation = getPreparedExplanationText({ explanationText, taskText });
  ...
  return `
    <div class="result-rich-text">
      <div class="result-text-flow">
        ${renderExplanationText(preparedExplanation)}
      </div>
      ${blocksHtml}
    </div>
  `;
