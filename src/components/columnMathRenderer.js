...
function trimNotes(notes, limit = 12) {
...
function normalizeDivisionNotes(notes) {
  ...
  trimmed.forEach((note, index) => {
    ...
    const stepLead = index === 0
      ? `Сначала берём первое неполное делимое ${payload.current}.`
      : `Теперь берём ${payload.current}.`;

    normalized.push({ ...note, text: stepLead });
    normalized.push({
      ...note,
      text: `Пишем ${payload.qDigit} в частном, потому что ${payload.qDigit} × ${payload.divisor} = ${payload.product}. Остаток ${payload.remainder}.`
    });
    ...
    if (nextPayload) {
      normalized.push({
        ...note,
        text: `Сносим следующую цифру и получаем ${nextPayload.current}.`
      });
    } else if (payload.remainder === '0') {
      normalized.push({ ...note, text: 'Деление закончено без остатка.' });
    } else {
      normalized.push({ ...note, text: `Получаем остаток ${payload.remainder}. Он меньше делителя, значит деление закончено.` });
    }
  });

  return trimNotes(normalized);
}
...
function prepareExplanationText(explanationText) {
  return String(explanationText || '');
}


function renderExplanationLine(line, index) {
  const safe = escapeHtml(line || '');
  const attrs = ` data-result-line-index="${index}"`;
  if (/^Ответ:/i.test(line)) {
...
function renderExplanationText(text) {
  ...
  return lines
    .map((line, index) => renderExplanationLine(line, index))
    .join('');
}
...
function buildPreparedExplanationData({ explanationText, taskText }) {
  ...
  const preparedExplanation = prepareExplanationText(String(explanationText || ''), operations, taskText);
  const explanationLines = splitExplanationLines(preparedExplanation);
  let nextLineIndex = 0;

  const blocks = operations.map(operation => {
    const model = buildColumnModel(operation);
    const titleLineIndex = explanationLines.length + nextLineIndex;
    nextLineIndex += 1;
    const noteItems = trimNotes(model.notes || []).map(note => {
      const lineIndex = explanationLines.length + nextLineIndex;
      nextLineIndex += 1;
  ...
  const speechLines = [...explanationLines];
  blocks.forEach(block => {
    speechLines.push(block.title);
    block.noteItems.forEach(note => speechLines.push(note.text));
  });
  ...
  return `
    <div class="result-rich-text">
      <div class="result-text-flow">
        ${renderExplanationText(data.preparedExplanation)}
      </div>
      ${blocksHtml}
    </div>
  `;
