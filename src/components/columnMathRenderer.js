...
function buildSubtractionModel(a, b) {
  ...
  for (let index = width - 1; index >= 0; index -= 1) {
    ...
    if (stepValue < 0) {
      ...
      notes.push(makeNote(`${effectiveTop} меньше ${bottomDigit}, занимаем 1 в старшем разряде`, color));
    }
  ...
  if (negative) {
    notes.unshift(makeNote('Первое число меньше второго, поэтому ответ будет отрицательным.', '#c50e03'));
  }
    ...
    tableHtml: `
      <table class="column-table arithmetic-table" aria-label="Умножение столбиком">
        <tbody>
          ...
          <tr>
            ${renderSignCell('')}
            ${Array.from({ length: width }, (_, index) => renderDigitCell('', { borderTop: true })).join('')}
          </tr>
          ...
          <tr>
            ${renderSignCell('')}
            ${Array.from({ length: width }, (_, index) => renderDigitCell('', { borderTop: true, className: 'final-line' })).join('')}
          </tr>
          <tr>
            ${renderSignCell('')}
...
function buildDivisionModel(a, b) {
  ...
  if (!steps.length) {
    notes.push(makeNote('Делимое меньше делителя, поэтому в ответе 0 и остаток равен делимому.', '#c50e03'));
  }
  ...
  const topRow = `
    <tr>
      ${renderSignCell('')}
      ${padDigits(dividendStr, dividendWidth).map(digit => renderDigitCell(digit, { digitColors: false })).join('')}
...
function collectOperations(taskText, explanationText) {
  const ordered = [...extractOperations(taskText), ...extractOperations(explanationText)];
  const seen = new Set();
  const result = [];
