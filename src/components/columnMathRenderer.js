...
function renderBlockShell({ title, titleLineIndex, inlineText = '', tableHtml, noteItems = [] }) {
  ...
  const inlineHtml = inlineText ? `<div class="column-card-subtitle">${escapeHtml(inlineText)}</div>` : '';
  return `
    <section class="column-card">
      ...
      <div class="column-card-body">
        ...
        <div class="column-notes">
          <div class="column-notes-title">Пояснения по шагам</div>
...
function formatMethodTitle(operator) {
  if (operator === '+') return 'Письменное сложение (сложение столбиком)';
  if (operator === '-') return 'Письменное вычитание (вычитание столбиком)';
  if (operator === '×') return 'Письменное умножение (умножение столбиком)';
  return 'Письменное деление (деление столбиком)';
}
...
function defaultAdviceForOperation(operation) {
  if (operation.operator === '+') return 'единицы записывай под единицами и начинай сложение с единиц';
  if (operation.operator === '-') return 'если цифры не хватает, занимаем 1 у соседнего разряда';
  if (operation.operator === '×') return 'умножай по разрядам справа налево и переноси десятки';
  return 'в делении столбиком повторяй шаги: взял, подобрал, умножил, вычел, снес следующую цифру';
}

function buildOperationLeadLines(operation) {
  if (operation.operator === '+') {
    return shouldUseColumn(operation)
      ? ['Ищем сумму.', 'Записываем единицы под единицами и начинаем сложение с единиц.']
      : ['Ищем сумму.', `Прибавляем ${operation.b} к ${operation.a}.`];
  }
  if (operation.operator === '-') {
    return shouldUseColumn(operation)
      ? ['Ищем разность.', 'Записываем единицы под единицами и вычитаем с единиц. Если цифры не хватает, занимаем 1 у соседнего разряда.']
      : ['Ищем разность.', `Вычитаем ${operation.b} из ${operation.a}.`];
  }
  if (operation.operator === '×') {
    return shouldUseColumn(operation)
      ? ['Ищем произведение.', 'Умножаем по разрядам справа налево и при необходимости переносим десятки.']
  ...
  return shouldUseColumn(operation)
    ? ['Ищем частное.', 'Берем первое неполное делимое, подбираем цифру частного, умножаем, вычитаем и сносим следующую цифру.']
...
function buildDirectOperationExplanation(operation, explanationText) {
  ...
  metaLines.push(`Совет: ${ensureSentenceEnding(parsed.adviceLine || adviceText)}`);
