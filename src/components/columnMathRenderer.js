...
function normalizeTeachingNotes(notes, operator, operation = null) {
  ...
  const normalizeMappedNotes = formatter => source
    .map(note => ({ ...note, text: formatter(note.text) }))
...
function isFlowBoundaryLine(text) {
  ...
  return /^(?:\d+\)|Ответ:|Совет:|Проверка:|Пример:|Порядок действий:?$|Решение(?: по действиям)?[.:]?$)/i.test(line);
}
