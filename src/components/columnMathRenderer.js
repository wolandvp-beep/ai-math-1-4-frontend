...
function renderBlockShell({ title, titleLineIndex, inlineText = '', tableHtml, noteItems = [] }) {
  ...
  const inlineHtml = String(inlineText || '').trim()
    ? `<div class="column-inline-expression">${escapeHtml(String(inlineText || '').trim())}</div>`
    : '';
  return `
...
function decorateColumnModel(model, operation) {
  return {
    ...
    inlineText: String(model.inlineText || '').trim(),
