import { renderStructuredExplanation } from '../utils/columnMath.js';
...
export function renderSolveScreen({ state }) {
  ...
  const resultHtml = hasResult
    ? renderStructuredExplanation(state.currentResult, state.draft || '')
    : '<div class="result-text-block"><p class="result-placeholder">Здесь появится объяснение и ответ.</p></div>';
  ...
  return `
    <section class="screen ${state.route === 'solve' ? 'active' : ''}" data-screen="solve">
      ...
      <div class="card answer-card result-card">
        ...
        <div id="resultBox" class="result result-rich">${resultHtml}</div>
