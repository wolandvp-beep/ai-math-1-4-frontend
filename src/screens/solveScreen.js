export function renderSolveScreen({ state }) {
  const result = (state.currentResult || '').trim();
  const hasResult = Boolean(result);
  const isLoading = result === '🤖 Думаю...';
  const examples = [
  ...
  return `
    <section class="screen ${state.route === 'solve' ? 'active' : ''}" data-screen="solve">
      <div class="card glass-card solve-hero-card">
        <div class="solve-hero-art" aria-hidden="true"></div>
        <div class="solve-topline">Решайка</div>
        <div class="solve-kicker">AI • Математика • Голографический режим</div>
        <div class="solve-main-subtitle">Введите условие задачи, пример, уравнение или математический вопрос.</div>
        <div class="input-shell">
          <textarea id="taskInput" class="input input-task" placeholder="${examples}" aria-label="Поле для ввода задачи">${state.draft || ''}</textarea>
        </div>
      ...
      <div class="card answer-card result-card">
        ...
        <div id="resultBox" class="result ${isLoading ? 'loading-result' : ''}">${hasResult ? state.currentResult : 'Здесь появится объяснение и ответ.'}</div>
