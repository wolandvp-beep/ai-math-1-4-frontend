...
import { renderMathPresentation } from '../components/mathLayouts.js';

export function renderSolveScreen({ state }) {
  const resultText = state.currentPresentation?.text || state.currentResult || '';
  ...
  return `
    <section class="screen ${state.route === 'solve' ? 'active' : ''}" data-screen="solve">
      ...
      <div class="card answer-card result-card">
        ...
        <div id="resultBox" class="result">${hasResult ? escapeHtml(resultText) : 'Здесь появится объяснение и ответ.'}</div>
        ${renderMathPresentation(state.currentPresentation)}
