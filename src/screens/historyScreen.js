function filterItems(state) {
  ...
  if (q) {
    items = items.filter(item =>
      ...
      String(item.resultText || item.result || '').toLowerCase().includes(q)
    );
  }
        ...
        ${items.length ? `<div class="list">
          ${items.map(item => `
            <button class="item item-history item-history-card" type="button" data-history-open="${item.id}" aria-label="Открыть объяснение от ${formatDateTime(item.createdAt)}">
              ...
              <div class="muted small">${String(item.resultText || item.result || '').slice(0, 180)}${String(item.resultText || item.result || '').length > 180 ? '…' : ''}</div>
            </button>
          `).join('')}
