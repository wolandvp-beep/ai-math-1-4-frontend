  ...
  return `
    <section class="screen ${state.route === 'history' ? 'active' : ''}" data-screen="history">
      <div class="card glass-card">
        ...
        <p class="section-sub">Здесь сохраняются последние объяснения. Нажмите «Старт», чтобы вернуть задачу и ответ.</p>
          ...
          ${items.map(item => `
            <div class="item item-history">
              ...
              <div class="item-actions">
                ...
                <button class="history-open-btn" data-history-open="${item.id}">Старт</button>
              </div>
            </div>
          `).join('')}
