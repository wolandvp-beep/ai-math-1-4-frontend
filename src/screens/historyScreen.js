...
export function renderHistoryScreen({ state }) {
  ...
  return `
    <section class="screen ${state.route === 'history' ? 'active' : ''}" data-screen="history">
      <div class="card glass-card premium-card history-shell skyline-scene-card skyline-scene-history-head v24-history-hero">
        <div class="v24-history-hero-grid">
          <div>
      ...
      <div class="card answer-card premium-card history-archive-card skyline-scene-card skyline-scene-history-grid v24-history-grid-shell">
        ${items.length ? `
          <div class="list premium-list archive-grid skyline-archive-grid v24-archive-grid">
            ${items.map((item, index) => {
              const kind = detectKind(item.task);
              return `
                <div class="item item-history premium-item archive-item skyline-archive-item v24-archive-item v24-kind-${kind.toLowerCase()}">
                  <div class="v24-archive-top">
                  ...
                  <div class="v24-archive-bottom">
                    <div class="v24-archive-signal"><span></span><span></span><span></span></div>
