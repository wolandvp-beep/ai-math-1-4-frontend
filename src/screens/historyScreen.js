...
export function renderHistoryScreen({ state }) {
  ...
  const isAll = state.historyMode !== 'favorites';
  const isFavorites = state.historyMode === 'favorites';
  ...
  return `
    <section class="screen ${state.route === 'history' ? 'active' : ''}" data-screen="history">
      <div class="card glass-card">
        ...
        <div class="row history-filters">
          <button id="historyAllBtn" class="secondary ${isAll ? 'is-active' : ''}" type="button">Все</button>
          <button id="historyFavBtn" class="secondary ${isFavorites ? 'is-active' : ''}" type="button">Избранное</button>
        </div>
      </div>
      ...
      <div class="card answer-card">
        ${items.length ? `<div class="list">
          ...
          `).join('')}
        </div>` : `<div class="empty">${isFavorites ? 'Пока нет избранных объяснений.' : 'Пока история пуста. Решите первую задачу — и она появится здесь.'}</div>`}
      </div>
    </section>
  `;
}
