...
function normalizeMathForSpeech(text) {
  ...
  normalized = normalized.replace(/([0-9A-Za-zА-Яа-яЁё)}\] ]+)\s*:\s*([0-9A-Za-zА-Яа-яЁё({\[ ]+)/g, (match, left, right) => {
    const normalizedLeft = left.trim();
    const normalizedRight = right.trim();
    const leftLooksMath = /(^|\s)(\d+|икс|[xхXХ]|скобка закрывается)$/.test(normalizedLeft) || /\d$/.test(normalizedLeft);
    const rightLooksMath = /^(\d+|икс|[xхXХ]|скобка открывается)(\s|$)/.test(normalizedRight) || /^\d/.test(normalizedRight);

    if (!leftLooksMath || !rightLooksMath) return match;
    return ` ${normalizedLeft} делить на ${normalizedRight} `;
  });
