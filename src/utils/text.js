...
function normalizeSectionLabel(line) {
  ...
  if (/^правило\s*:/i.test(trimmed)) {
    return `Правило: ${trimmed.replace(/^правило\s*:/i, '').trim()}`.trim();
  }
  if (/^краткое\s+условие\s*:/i.test(trimmed)) {
    return `Краткое условие: ${trimmed.replace(/^краткое\s+условие\s*:/i, '').trim()}`.trim();
  }
  if (/^решение\s*:/i.test(trimmed)) {
    return `Решение: ${trimmed.replace(/^решение\s*:/i, '').trim()}`.trim();
  }
...
function compactBodyLines(lines, hasCheck) {
  ...
  const maxBodyLines = hasCheck ? 6 : 7;
...
export function normalizeAssistantText(text) {
  ...
  let prepared = String(text)
    ...
    .replace(/^\s*(Запомни|Памятка)\s*[—:-]?\s*/gim, 'Правило: ')
    .replace(/\b(Запомни|Памятка)\b\s*[—:-]?/gi, 'Правило: ')
    .replace(/\s*(Краткое\s+условие\s*:)/gi, '\n$1')
    .replace(/\s*(Правило\s*:)/gi, '\n$1')
    .replace(/\s*(Решение\s*:)/gi, '\n$1')
  ...
  const parsedLines = prepared
    ...
    .map(line => line.replace(/^Правило:\s*Правило:\s*/i, 'Правило: '))
    .map(line => line.replace(/^Краткое условие:\s*Краткое условие:\s*/i, 'Краткое условие: '))
    .map(line => line.replace(/^Решение:\s*Решение:\s*/i, 'Решение: '));
