...
function compactBodyLines(lines, hasCheck) {
  ...
  const maxBodyLines = hasCheck ? 3 : 4;
  ...
  const hasResultAtEnd = /=\s*[^=]+$/.test(tail);
  if (hasResultAtEnd && !hasCheck) {
    const resultValue = tail.match(/=\s*([^=]+)$/)?.[1]?.trim();
    if (resultValue) {
      return [...body.slice(0, maxBodyLines - 1), `Дальше считаем по порядку и получаем ${resultValue}`];
    }
  }

  return [...body.slice(0, maxBodyLines - 1), tail];
}
