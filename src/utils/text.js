...
function compactBodyLines(lines, hasCheck) {
  ...
  const preserveDetailedFlow = body.some(line => /^\d+\)/.test(String(line || '').trim()))
    || body.some(line => /^(Спрашиваем:|Получаем:|Записываем:|Находим общий знаменатель|Определяем первое неполное делимое)/i.test(String(line || '').trim()));
