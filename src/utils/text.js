...
function isStructureLine(line) {
  return /^(?:Задача\.?|Решение\.?|Пример:|Уравнение:|Порядок действий:?|Решение по действиям:?|Что известно:|Что нужно найти:|Запись столбиком:?|Пояснение по шагам:?)/i.test(String(line || '').trim());
}

function normalizeStepPrefix(line) {
  const trimmed = String(line || '').trim();
  if (!trimmed) return '';

  const stepMatch = trimmed.match(/^Шаг\s*(\d+)\s*:?\s*(.+)$/i);
  if (stepMatch) {
    return `${stepMatch[1]}) ${stepMatch[2].trim()}`.trim();
  }

  const dotMatch = trimmed.match(/^(\d+)\.\s+(.+)$/);
  if (dotMatch) {
    return `${dotMatch[1]}) ${dotMatch[2].trim()}`.trim();
  }
...
function normalizeSectionLabel(line) {
  const trimmed = normalizeStepPrefix(line);
  if (!trimmed) return '';
...
function compactBodyLines(lines, hasCheck) {
  ...
  lines.forEach(line => {
    ...
    const dedupeKey = cleaned.toLowerCase().replace(/[.!?]+$/g, '');
    if (!isNumberedLine(cleaned) && seen.has(dedupeKey)) return;
    seen.add(dedupeKey);
    raw.push(cleaned);
  });

  const informativeExists = raw.some(line => hasMathSignal(line) || isNumberedLine(line));
  const normalized = raw.filter(line => !(informativeExists && !isNumberedLine(line) && GENERIC_BODY_LINE.test(line)));
  const body = normalized.length ? normalized : raw;
  const structured = body.some(line => isNumberedLine(line) || isStructureLine(line));
  const maxBodyLines = structured ? 16 : (hasCheck ? 6 : 8);
...
export function normalizeAssistantText(text) {
  ...
  if (!adviceLine) adviceLine = 'Совет: решай по шагам и проверяй ответ';

  return [...compactBody, ...(checkLine ? [checkLine] : []), answerLine, adviceLine].join('\n').trim();
}
